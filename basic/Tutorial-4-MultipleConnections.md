...
```python
            if flag & select.POLLOUT:
                try:
                    ...
                    s.send(next_msg)
```
If the `Queue` for socket `s` is empty, this will trigger a `queue.Empty` _exception_, causing the `poller.modify(s, select.POLLIN)` line to be executed, which tells `poller` to go back to only notifying us about new incoming data ready on the socket `s`. Otherwise, the `else` clause is executed, when sends the data stored in `next_msg` out on the ready socket `s`, but leaves `poller` to also notify us when `s` is ready to send additional outgoing data again.
